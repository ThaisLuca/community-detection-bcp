...
def set_feats(bcp_features, bcp_example):
    ...
    example = np.zeros((len(bcp_features)), dtype=bool)
    example[feat_idx] = 1
    return example
...
def get_features(bcp_examples):
    ...
    print(len(bcp_examples),len(bcp_features))
    ...
    #all_examples = pd.DataFrame([], columns=bcp_features)
    for i in range(0, len(bcp_examples), leap):
        batch = bcp_examples[i:i+leap]
        with Pool(20) as p:
            examples = p.map(set_feats_p, batch)

        examples = np.stack(examples, axis=0)
        #all_examples = pd.concat((all_examples, pd.DataFrame(examples, columns=bcp_features)), axis=0)
        pd.DataFrame(examples, columns=bcp_features).to_csv(f'features.csv', mode='a',index=False) 
        del batch, examples
